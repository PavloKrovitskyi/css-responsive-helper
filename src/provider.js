...
module.exports = class CSSResponsiveProvider {
  ...
  provideCompletionItems(document, position) {
    return new Promise((resolve, reject) => {
      ...
      // Get the unit order from configuration
      const config = vscode.workspace.getConfiguration('CSSResponsiveHelper');
      const unitOrder = config.get('unitOrder', ['clamp', 'rem', 'percent', 'natural', 'em']);

      // Available units
      const availableUnits = ['clamp', 'rem', 'percent', 'natural', 'em'];

      // Create ordered list, ensuring all units are included
      const orderedUnits = [];
      const usedUnits = new Set();

      // Add units in user-specified order
      unitOrder.forEach((unit) => {
        if (availableUnits.includes(unit) && !usedUnits.has(unit)) {
          orderedUnits.push(unit);
          usedUnits.add(unit);
        }
      });

      // Add remaining units not specified by user
      ...
      // Create keysInOrder with sortText
      const keysInOrder = orderedUnits.map((key, index) => ({
        key,
        sortText: index.toString().padStart(2, '0'), // Ensure proper sorting, e.g., '00', '01', etc.
      }));
